classdef Data < handle
    ...
    properties
        ...
        gtTempo                         % ground truth tempo (in beats per min)
        gtAnn                           % Ground truth example annotations (downbeats mainly)
...
%             tempo_max_per_cluster = cellfun(@max, tempo_max_accum);
        ...
        function obj = read_gtTempo(obj)
            for iFile = 1:length(obj.file_list)
                [obj.gtTempo{iFile}, ~ ] = Data.load_annotations_bt(obj.file_list{iFile}, 'gtbpm');
            end
        end
        function obj = read_gtAnn(obj)
            for iFile = 1:length(obj.file_list)
                [obj.gtAnn{iFile}, ~ ] = Data.load_annotations_bt(obj.file_list{iFile}, 'gtsama');
            end
        end
