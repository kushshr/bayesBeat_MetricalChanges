...
sup_ext = {'.beats', '.meter', '.onsets', '.bpm', '.dancestyle', '.rhythm', '.section', '.gtbpm', '.gtsama'};
...
elseif strcmp(ext, '.beats') || strcmp(ann_type, 'beats')
    ...
    data = temp{1};
    filename_meter = strrep(filename,'beats','meterlevel');
    fidm = fopen(filename_meter,'r');
    temp2 = textscan(fidm, '%f64%s', 'delimiter', '\t');
    data =[data temp2{1}];
    %if size(temp, 2) == 2
...
elseif strcmp(ext, '.bpm') || strcmp(ann_type, 'bpm')
    % Load Tempofile
    data = load('-ascii', filename);
elseif strcmp(ext, '.gtbpm') || strcmp(ann_type, 'gtbpm')
    % Load Ground truth Tempofile
    data = load('-ascii', filename);
elseif strcmp(ext, '.gtsama') || strcmp(ann_type, 'gtsama')
    % Load ground truth downbeat file
    data = load('-ascii', filename);    
