...
for iFile=1:nFiles
    ...
    if error1
        ...
        Output.file2meter = cat(1,Output.file2meter,meter)
    end
    if error2
        for kPatt = 1:size(meter,1)
            iSection = iSection + 1; 
            Output.file2section{iSection} = 1;
            Output.sec_len{iSection} = meter(kPatt);
            max_sec_len(iSection) = meter(kPatt);
            fprintf('    WARNING: Section information not read, assuming bar length sections\n');
        end
    else    
...
for iFile=1:nFiles
    ...
    for kPatt = 1:size(meter,1)
        ...
        Output.fileNames{foobar} = listing(iFile).name;
        ...
        if (length(b1) <= 1) && strcmp(pattern_size, 'bar')
            ...
            if strcmp(pattern_size, 'bar')
                bar_grid_eff = ceil(whole_note_div * meter(kPatt,1) ./meter(kPatt, 2));
            elseif strcmp(pattern_size, 'section')
                bar_grid_eff = ceil(whole_note_div * Output.sec_len{iFile} ./ meter(kPatt, 2));
            else
            ...
            [pattData, nchar, Output.patt_pos_per_frame{foobar}, Output.patt_num_per_frame{foobar},...
                Output.sec_label_per_frame{foobar}, Output.bar_num_per_frame{foobar}, Output.sec_label_per_pattern{foobar}] = ...
                get_feature_at_bar_grid(wav_fln, feature_type, beats_all{iFile}, whole_note_div, bar_grid_eff, ...
                frame_length, pattern_size, meter(kPatt, :), ...
                Output.file2section{foobar}, Output.sec_len{foobar}, nchar);
            if ~isempty(pattData)
                [nNewPatts, currBarGrid] = size(pattData);
                % If there are any empty cells, fill it up with the neighbor
                % value: Added to handle singular cases - new method
                cellInd = cellfun(@isempty,pattData);
                for ii = 1:nNewPatts
                    cellInd(ii,bar_grid_eff(Output.sec_label_per_pattern{foobar}(ii))+1:end) = 0; 
                end
                pattData(cellInd) = {nanmean(nanmean(cellfun(@mean,pattData)))};
                if sum(cellInd(:))
                        disp('Some data grid value set to mean value due to lack of data...');
                end
                clear cellInd
                % If there are any empty cells, fill it up with the neighbor
                % value: Added to handle singular cases: older method here
                % pattData(cellfun(@isempty,pattData)) = {nanmean(nanmean(cellfun(@mean,pattData)))};
                ...
                Output.patt2file((idLastPatt + 1):(idLastPatt + nNewPatts)) = foobar;
                Output.patt2sec((idLastPatt + 1):(idLastPatt + nNewPatts)) = Output.sec_label_per_pattern{foobar}(:);
                Output.patt_num_per_frame{foobar} = Output.patt_num_per_frame{foobar} + idLastPatt;
                Output.patt_len((idLastPatt + 1):(idLastPatt + nNewPatts)) = Output.sec_len{foobar}(Output.sec_label_per_pattern{foobar}(:));
