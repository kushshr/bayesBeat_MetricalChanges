    ...
    methods
        function obj = TransitionModel(M_per_pattern, N, R, pn, pr, alpha, ...
            ...
            if strcmp(obj.tm_type, 'whiteley')
                ...
                if sum(obj.minN == 0)
                    obj.minN(obj.minN == 0) = 1;
                    disp('Min tempo in dataset is smaller than minimum that can be tracked. Setting minN = 1');
                end
                obj.maxN = ceil(position_states_per_beat .* ...
                    obj.frame_length .* obj.max_bpm ./ 60);
                if sum(obj.maxN == 1)
                    obj.maxN(obj.maxN == 1) = 2;
                    disp('Max tempo in dataset is smaller than max that can be tracked. Setting maxN = 2');
                end
