classdef HMM
    ...
    methods
        function obj = HMM(Params, data)
            ...
            obj.rhythm2meter = obj.rhythm2meter;
        ...
        function results = do_inference(obj, y, fname, inference_method, varargin)
            ...
            if isempty(strfind(inference_method, 'HMM'))
                error('Inference method %s not compatible with HMM model\n', inference_method);
            end
            ...
            % Add one more col at end for compatibility with SPM
            beats = [beats ones(size(beats,1),1)];
            ...
            results{3} = meter;
            results{4} = r_path;
            results{5} = r_path;   % Dummy section store
            results{6} = (1:size(y,1) - 0.5) * obj.frame_length;
            results{7} = hidden_state_sequence;
            % Clear old variables
            clear gtTempo gtSama
        end
